...
knnFit$finalModel
```
For each k value from 1 to 10, the accuracy and Kappa statistics is given. The best k value is chosen based on the highest accuracy, and in this case, the best k is 1 (k = 1) due it's highest value.
...
rulesFit
```
We have included the accuracy and Kappa statistics for each combination of `threshold` and `pruned` settings. Here, it is 1 in every circumstance. The model's final values were pruned = yes and threshold = 0.5.

```{r}
rulesFit$finalModel
```
This decision list serves as a classification tool and offers guidelines for grouping instances into classes.
...
svmFit$finalModel
```
Training error: 0 indicates that the training error for the SVM model is 0, meaning the model perfectly fits the training data.
...
```
The number of predictors chosen at random to split a node in a Random Forest tree is indicated by the `mtry` parameter. Based on the highest accuracy, the best `mtry` value was chosen; in this instance, that number is 19. With just one incorrect classification, class `Gardner` had an extremely low error rate of about 0.000663; in contrast, class `Janson` properly classified every occurrence.
...
```
The results indicate that the model achieved very high accuracy (0.998) and Kappa (0.995).
...
nnetFit
```
During hyperparameter tuning, several combinations of `size` and `decay` values are explored to see which combination yields the greatest model performance. The ideal model was chosen using accuracy, and the model's final values are size = 7 and decay = 0.01.

```{r}
nnetFit$finalModel
```
Output provides a detailed summary of the neural network architecture, the input features, the output etc.
...
summary(resamps)
```
The accuracy values range from 0.819 to 1.00 and Kappa values ranges from 0.648 to 1.000, showing majority of models are performing well.
...
```
Calculating a confusion matrix for the held-out test data.
...
ggplot(x, aes(x = edition_number, y = year, fill = book)) +  
  ...
  labs(x = "Edition number",
       ...
       alpha = "Density")
```
Plotting the link between `year` and `edition_number` is done using different colours to indicate the "book." Transparency is used to highlight denser areas.
...
The plot displays the decision boundaries of the Naive Bayes classifier.
...
```
The use of logistic regression to situations involving more than two classes is known as multinomial logistic regression.The output provides details about the optimisation iterations, including the initial value, values that decrease iteratively, and the converged value at the end. It also shows that after 100 iterations, the optimisation process was stopped.
...
decisionplot(model, x, class_var = "book") + 
  labs(title = "Multinomial Logistic Regression",
      x = "Edition number",
       ...
       fill = "Prediction")
```
The plot show the decision boundaries and how the model separates or classifies data points into different classes.
...
decisionplot(model, x, class_var = "book") + 
  labs(title = "CART (overfitting)",
       ...
       fill = "Prediction")
```
Parameter (cp = 0.001) and a minimum split of 1 (minsplit = 1) can cause overfitting, but it's not showing much changes in the plot.
...
decisionplot(model, x, class_var = "book") + 
  labs(title = "C5.0",
       ...
       fill = "Prediction")
```
The decision plot typically shows the decision boundaries and how the C5.0 model categorizes different data points.
...
The `polynomial` kernel is a non-linear kernel that works well for non-linear classification jobs because it can identify intricate patterns in the data.When data correlations are non-linear and may show intricate patterns, sigmoid kernels might be helpful.
